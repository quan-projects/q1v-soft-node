...
namespace {
    ...
    static bool print_as_json(const T &obj) {
        std::cout << Q1v::storeToJson(obj) << ENDL;
        return true;
    }
}

DaemonCommandsHandler::DaemonCommandsHandler(Q1v::core &core, Q1v::NodeServer &srv, Logging::LoggerManager &log)
    ...
    m_consoleHandler.setHandler("print_cn", boost::bind(&DaemonCommandsHandler::print_cn, this, _1),
                                "Print connections");
...
std::string DaemonCommandsHandler::get_commands_str() {
    std::stringstream ss;
    ss << Q1v::CRYPTONOTE_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
    ss << "Commands: " << ENDL;
...
bool DaemonCommandsHandler::print_block_by_height(uint32_t height) {
    ...
    if (1 == blocks.size()) {
        ...
        std::cout << "block wasn't found. Current block chain height: " << current_height << ", requested: " << height
                  << std::endl;
        return false;
    }
