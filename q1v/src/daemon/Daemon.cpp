...
namespace {
    const command_line::arg_descriptor<std::string> arg_config_file = {"config-file", "Specify configuration file",
                                                                       std::string(Q1v::CRYPTONOTE_NAME) + ".conf"};
    const command_line::arg_descriptor<bool> arg_os_version = {"os-version", ""};
    const command_line::arg_descriptor<std::string> arg_log_file = {"log-file", "", ""};
    const command_line::arg_descriptor<int> arg_log_level = {"log-level", "", 2}; // info level
    const command_line::arg_descriptor<int> arg_cache_size = {"cache-size",
                                                              "Number of blocks to keep in cache. Default is 2048",
                                                              2048}; // info level
    ...
    const command_line::arg_descriptor<std::string> arg_enable_cors = {"enable-cors",
                                                                       "Adds header 'Access-Control-Allow-Origin' to the daemon's RPC responses. Uses the value as domain. Use * for all",
                                                                       ""};
    const command_line::arg_descriptor<std::string> arg_set_fee_address = {"fee-address",
                                                                           "Sets fee address for light wallets to the daemon's RPC responses.",
                                                                           ""};
    const command_line::arg_descriptor<std::string> arg_set_wn_message = {"wn-message",
                                                                          "Text message for light wallets. Use it to promote your wallet node. Max 70 characters.",
                                                                          ""};
    const command_line::arg_descriptor<double_t> arg_set_fee_percent = {"fee",
                                                                        "Sets fee percentage for light wallets. Format float. Default is 0.25.",
                                                                        0.25};
    const command_line::arg_descriptor<bool> arg_testnet_on = {"testnet",
...
void print_genesis_tx_hex(const po::variables_map &vm, LoggerManager &logManager) {
    Q1v::Transaction tx = Q1v::CurrencyBuilder(logManager).generateGenesisTransaction();
    std::string tx_hex = Common::toHex(Q1v::toBinaryArray(tx));
    ...
    try {
        ...
        bool r = command_line::handle_error_helper(desc_options, [&]() {
            ...
            if (command_line::get_arg(vm, command_line::arg_help)) {
                std::cout << Q1v::CRYPTONOTE_NAME << " v" << PROJECT_VERSION_LONG << ENDL << ENDL;
        ...
        logger(INFO) << Q1v::CRYPTONOTE_NAME << " v" << PROJECT_VERSION_LONG;
        ...
        Q1v::CurrencyBuilder currencyBuilder(logManager);
        currencyBuilder.testnet(testnet_mode);
        try {
            currencyBuilder.currency();
        } catch (std::exception &) {
            std::cout << "GENESIS_COINBASE_TX_HEX constant has an incorrect value. Please launch: "
                      << Q1v::CRYPTONOTE_NAME << " --" << arg_print_genesis_tx.name;
            return 1;
        }
        ...
        for (const auto &cp: Q1v::CHECKPOINTS) {
        ...
        Q1v::CryptoNoteProtocolHandler cprotocol(currency, dispatcher, ccore, nullptr, logManager);
        Q1v::NodeServer p2psrv(dispatcher, cprotocol, logManager);
        Q1v::RpcServer rpcServer(dispatcher, logManager, ccore, p2psrv, cprotocol);
...
bool command_line_preprocessor(const boost::program_options::variables_map &vm, LoggerRef &logger) {
    ...
    if (command_line::get_arg(vm, command_line::arg_version)) {
        std::cout << Q1v::CRYPTONOTE_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
        exit = true;
    }
