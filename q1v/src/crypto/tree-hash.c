...
void tree_hash(const char (*hashes)[HASH_SIZE], size_t count, char *root_hash) {
    ...
    if (count == 1) {
        ...
        q1v_fast_hash(hashes, 2 * HASH_SIZE, root_hash);
    } else {
        size_t i, j;
        size_t cnt = count - 1;
        char (*ints)[HASH_SIZE];
        ...
        for (i = 2 * cnt - count, j = 2 * cnt - count; j < cnt; i += 2, ++j) {
            q1v_fast_hash(hashes[i], 2 * HASH_SIZE, ints[j]);
        }
        assert(i == count);
        while (cnt > 2) {
            cnt >>= 1;
            for (i = 0, j = 0; j < cnt; i += 2, ++j) {
                q1v_fast_hash(ints[i], 2 * HASH_SIZE, ints[j]);
            }
        }
        q1v_fast_hash(ints[0], 2 * HASH_SIZE, root_hash);
    }
}
    ...
    if (depth == 0) {
        ...
        while (depth > 0) {
            ...
            if (from_leaf) {
                ...
                q1v_fast_hash(buffer, 2 * HASH_SIZE, leaf_path);
            }
            memcpy(branch_path, branch[depth], HASH_SIZE);
        }
        q1v_fast_hash(buffer, 2 * HASH_SIZE, root_hash);
    }
}
