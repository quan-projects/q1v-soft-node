        ...
        if (result != 0) {
            if (result == 1) {
                ...
                if (UPNP_AddPortMapping(urls.controlURL, igdData.first.servicetype, portString.str().c_str(),
                                        portString.str().c_str(), lanAddress, Q1v::CRYPTONOTE_NAME, "TCP", 0, "0") !=
                    0) {
...
namespace Q1v {
    namespace {
        const command_line::arg_descriptor<std::string> arg_p2p_bind_ip = {"p2p-bind-ip",
                                                                           "Interface for p2p network protocol",
                                                                           "0.0.0.0"};
        const command_line::arg_descriptor<std::string> arg_p2p_bind_port = {"p2p-bind-port",
                                                                             "Port for p2p network protocol",
                                                                             std::to_string(Q1v::P2P_DEFAULT_PORT)};
        const command_line::arg_descriptor<uint32_t> arg_p2p_external_port = {"p2p-external-port",
                                                                              "External port for p2p network protocol (if port forwarding used with NAT)",
                                                                              0};
        const command_line::arg_descriptor<bool> arg_p2p_allow_local_ip = {"allow-local-ip",
        ...
        const command_line::arg_descriptor<std::vector<std::string> > arg_p2p_add_priority_node = {"add-priority-node",
        ...
        const command_line::arg_descriptor<std::vector<std::string> > arg_p2p_seed_node = {"seed-node",
                                                                                           "Connect to a node to retrieve peer addresses, and disconnect"};
        const command_line::arg_descriptor<bool> arg_p2p_hide_my_port = {"hide-my-port",
        ...
        std::string print_peerlist_to_string(const std::list<PeerlistEntry> &pl) {
            ...
            for (const auto &pe: pl) {
                ss << pe.id << "\t" << pe.adr << " \tlast_seen: "
                   << Common::timeIntervalToString(now_time - pe.last_seen) << std::endl;
            }
            return ss.str();
        }
    }
    ...
    uint64_t P2pConnectionContext::writeDuration(TimePoint now) const { // in milliseconds
        return writeOperationStartTime == TimePoint() ? 0 : std::chrono::duration_cast<std::chrono::milliseconds>(
                now - writeOperationStartTime).count();
    }
    ...
    NodeServer::NodeServer(System::Dispatcher &dispatcher, Q1v::CryptoNoteProtocolHandler &payload_handler,
                           Logging::ILogger &log) : m_dispatcher(dispatcher), m_workingContextGroup(dispatcher),
                                                    m_payload_handler(payload_handler), m_allow_local_ip(false),
                                                    m_hide_my_port(false), m_network_id(Q1V_NETWORK),
                                                    logger(log, "node_server"), m_stopEvent(m_dispatcher),
                                                    m_idleTimer(m_dispatcher), m_timedSyncTimer(m_dispatcher),
                                                    m_timeoutTimer(m_dispatcher), m_stop(false),
            // intervals
            // m_peer_handshake_idle_maker_interval(Q1v::P2P_DEFAULT_HANDSHAKE_INTERVAL),
...
#undef INVOKE_HANDLER
        ...
        try {
            ...
            try {
                ...
                if (!p2p_data.fail()) {
                    ...
                    Q1v::serialize(*this, a);
                    loaded = true;
                }
            } catch (const std::exception &e) {
                logger(ERROR, BRIGHT_RED) << "Failed to load config from file '" << state_file_path << "': "
                                          << e.what();
            }
            ...
            m_config.m_net_config.handshake_interval = Q1v::P2P_DEFAULT_HANDSHAKE_INTERVAL;
            m_config.m_net_config.connections_count = Q1v::P2P_DEFAULT_CONNECTIONS_COUNT;
            m_config.m_net_config.packet_max_size = Q1v::P2P_DEFAULT_PACKET_MAX_SIZE; //20 MB limit
            m_config.m_net_config.config_id = 0; // initial config
            m_config.m_net_config.connection_timeout = Q1v::P2P_DEFAULT_CONNECTION_TIMEOUT;
            m_config.m_net_config.ping_connection_timeout = Q1v::P2P_DEFAULT_PING_CONNECTION_TIMEOUT;
            m_config.m_net_config.send_peerlist_sz = Q1v::P2P_DEFAULT_PEERS_IN_HANDSHAKE;
    ...
    bool NodeServer::init(const NetNodeConfig &config) {
        ...
        // m_net_server.get_config_object().m_invoke_timeout = Q1v::P2P_DEFAULT_INVOKE_TIMEOUT;
        ...
        m_listener = System::TcpListener(m_dispatcher, System::Ipv4Address(m_bind_ip),
    ...
    Q1v::CryptoNoteProtocolHandler &NodeServer::get_payload_object() {
    ...
    //-----------------------------------------------------------------------------------
    bool NodeServer::handshake(Q1v::LevinProtocol &proto, P2pConnectionContext &context, bool just_take_peerlist) {
        ...
        if (rsp.node_data.network_id != m_network_id) {
            logger(Logging::ERROR) << context << "COMMAND_HANDSHAKE Failed, wrong network!  ("
        ...
        try {
            ...
            try {
                ...
                System::Context<> timeoutContext(m_dispatcher, [&] {
                    System::Timer(m_dispatcher).sleep(
                            std::chrono::milliseconds(m_config.m_net_config.connection_timeout));
            ...
            try {
                System::Context<bool> handshakeContext(m_dispatcher, [&] {
                    Q1v::LevinProtocol proto(ctx.connection);
                ...
                System::Context<> timeoutContext(m_dispatcher, [&] {
                    ...
                    System::Timer(m_dispatcher).sleep(
                            std::chrono::milliseconds(m_config.m_net_config.connection_timeout * 3));
            ...
            m_workingContextGroup.spawn(std::bind(&NodeServer::connectionHandler, this, std::cref(connectionId),
    ...
    bool NodeServer::connections_maker() {
        ...
        size_t expected_white_connections =
                (m_config.m_net_config.connections_count * Q1v::P2P_DEFAULT_WHITELIST_CONNECTIONS_PERCENT) / 100;
    ...
    bool NodeServer::handle_remote_peerlist(const std::list<PeerlistEntry> &peerlist, time_t local_time,
        ...
        if (!fix_time_delta(peerlist_, local_time, delta))
            return false;
        logger(Logging::TRACE) << context << "REMOTE PEERLIST: TIME_DELTA: " << delta << ", remote peerlist size="
                               << peerlist_.size();
    ...
    bool NodeServer::check_trust(const proof_of_trust &tr) {
        ...
        if (time_delata > 24 * 60 * 60) {
            logger(ERROR) << "check_trust failed to check time conditions, local_time=" << local_time << ", proof_time="
                          << tr.time;
            return false;
        }
        ...
        Crypto::PublicKey pk;
        Common::podFromHex(Q1v::P2P_STAT_TRUSTED_PUB_KEY, pk);
    ...
    void NodeServer::relay_notify_to_all(int command, const BinaryArray &data_buff,
        ...
        net_connection_id excludeId = excludeConnection ? *excludeConnection
                                                        : boost::value_initialized<net_connection_id>();
    ...
    bool NodeServer::try_ping(basic_node_data &node_data, P2pConnectionContext &context) {
        ...
        try {
            ...
            System::Context<> timeoutContext(m_dispatcher, [&] {
                System::Timer(m_dispatcher).sleep(
                        std::chrono::milliseconds(m_config.m_net_config.connection_timeout * 2));
    ...
    std::string NodeServer::print_connections_container() {
        ...
        for (const auto &cntxt: m_connections) {
            ss << Common::ipAddressToString(cntxt.second.m_remote_ip) << ":" << cntxt.second.m_remote_port
               ...
               << (cntxt.second.m_is_income ? " INC" : " OUT") << std::endl;
        }
