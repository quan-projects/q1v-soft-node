...
namespace Q1v {
    ...
    namespace {
        ...
        void doWithTimeoutAndThrow(System::Dispatcher &dispatcher, std::chrono::nanoseconds timeout,
                                   std::function<void()> f) {
            std::string result;
    ...
    P2pNode::P2pNode(const P2pNodeConfig &cfg, Dispatcher &dispatcher, Logging::ILogger &log,
                     const Crypto::Hash &genesisHash, PeerIdType peerId) : logger(log, "P2pNode:" + std::to_string(
            cfg.getBindPort())), m_stopRequested(false), m_cfg(cfg), m_myPeerId(peerId), m_genesisHash(genesisHash),
                                                                           m_genesisPayload(
                                                                                   CORE_SYNC_DATA{1, genesisHash}),
    ...
    bool P2pNode::makeNewConnectionFromPeerlist(const PeerlistManager::Peerlist &peerlist) {
        ...
        for (size_t tryCount = 0; idxGen.generate(peerIndex) && tryCount < m_cfg.getPeerListGetTryCount(); ++tryCount) {
            ...
            logger(DEBUGGING) << "Selected peer: [" << peer.id << " " << peer.adr << "] last_seen: "
                              << (peer.last_seen ? Common::timeIntervalToString(time(nullptr) - peer.last_seen)
                                                 : "never");
    ...
    P2pNode::ContextPtr P2pNode::tryToConnectPeer(const NetworkAddress &address) {
        try {
            ...
            doWithTimeoutAndThrow(m_dispatcher, m_cfg.getConnectTimeout(), [&] {
                tcpConnection = connector.connect(Ipv4Address(Common::ipAddressToString(address.ip)),
            ...
            return ContextPtr(
                    new P2pContext(m_dispatcher, std::move(tcpConnection), false, address, m_cfg.getTimedSyncInterval(),
                                   getGenesisPayload()));
        } catch (std::exception &e) {
    ...
    bool P2pNode::fetchPeerList(ContextPtr connection) {
        try {
            ...
            if (response.node_data.network_id != request.node_data.network_id) {
                logger(ERROR) << *connection << "COMMAND_HANDSHAKE failed, wrong network: "
