    ...
    namespace JsonRpc {
        ...
        public:
            ...
            bool loadParams(T &v) const {
                loadFromJsonValue(v, psReq.contains("params") ? psReq("params") : Common::JsonValue(
                        Common::JsonValue::NIL));
                return true;
            }
        ...
        bool invokeMethod(const JsonRpcRequest &jsReq, JsonRpcResponse &jsRes, Handler handler) {
            ...
            if (!std::is_same<Request, Q1v::EMPTY_STRUCT>::value && !jsReq.loadParams(req)) {
        ...
        JsonMemberMethod makeMemberMethod(bool (Class::*handler)(const Params &, Result &)) {
            return [handler](void *obj, const JsonRpcRequest &req, JsonRpcResponse &res) {
                return JsonRpc::invokeMethod<Params, Result>(req, res, std::bind(handler, static_cast<Class *>(obj),
                                                                                 std::placeholders::_1,
                                                                                 std::placeholders::_2));
            };
        }
