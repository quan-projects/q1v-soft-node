...
namespace Q1v {
    ...
    bool appendMergeMiningTagToExtra(std::vector<uint8_t> &tx_extra, const TransactionExtraMergeMiningTag &mm_tag) {
        ...
        std::copy(reinterpret_cast<const uint8_t *>(blob.data()),
                  reinterpret_cast<const uint8_t *>(blob.data() + blob.size()), std::back_inserter(tx_extra));
        return true;
    }
    ...
    bool createTxExtraWithPaymentId(const std::string &paymentIdString, std::vector<uint8_t> &extra) {
        ...
        Q1v::setPaymentIdToTransactionExtraNonce(extraNonce, paymentIdBin);

        if (!Q1v::addExtraNonceToTransactionExtra(extra, extraNonce)) {
            return false;
        }
