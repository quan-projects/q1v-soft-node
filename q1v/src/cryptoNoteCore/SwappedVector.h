...
void SwappedVector<T>::close() {
    std::cout << "SwappedVector cache hits: " << m_cacheHits << ", misses: " << m_cacheMisses << " (" << std::fixed
...
const T &SwappedVector<T>::operator[](uint64_t index) {
    ...
    Q1v::BinaryInputStreamSerializer archive(stream);
...
template<class T>
void SwappedVector<T>::clear_cache() {
    m_items.clear();
    m_cache.clear();
}
...
void SwappedVector<T>::push_back(const T &item) {
...

    {
        ...
        Q1v::BinaryOutputStreamSerializer archive(stream);
