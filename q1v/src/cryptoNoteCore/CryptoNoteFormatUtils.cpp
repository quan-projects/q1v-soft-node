...
namespace Q1v {
    ...
    bool parseAndValidateTransactionFromBinaryArray(const BinaryArray &tx_blob, Transaction &tx, Hash &tx_hash,
                                                    Hash &tx_prefix_hash) {
        ...
        //TODO: validate tx
        q1v_fast_hash(tx_blob.data(), tx_blob.size(), tx_hash);
    ...
    bool
    constructTransaction(const AccountKeys &sender_account_keys, const std::vector<TransactionSourceEntry> &sources,
        ...
        for (const TransactionSourceEntry &src_entr: sources) {
            if (src_entr.realOutput >= src_entr.outputs.size()) {
                logger(ERROR) << "real_output index (" << src_entr.realOutput << ")bigger than output_keys.size()="
        ...
        for (const TransactionDestinationEntry &dst_entr: shuffled_dsts) {
            ...
            if (!(r)) {
                logger(ERROR, BRIGHT_RED) << "at creation outs: failed to generate_key_derivation("
    ...
    bool get_block_longhash(q1v_pow_hash_v2 &ctx, const Block &b, Hash &res) {
        BinaryArray bd;
        ...
        if (b.majorVersion < BLOCK_MAJOR_VERSION_5) {
            ...
            q1v_pow_hash_v1 ctx_v1 = q1v_pow_hash_v1::make_borrowed(ctx);
