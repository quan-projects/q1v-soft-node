...
namespace Q1v {
    ...
    miner::miner(const Currency &currency, IMinerHandler &handler, Logging::ILogger &log) : m_currency(currency),
                                                                                            logger(log, "miner"),
                                                                                            m_stop(true), m_template(
                    boost::value_initialized<Block>()), m_template_no(0), m_diffic(0), m_handler(handler),
                                                                                            m_pausers_count(0),
                                                                                            m_threads_total(0),
                                                                                            m_starter_nonce(0),
    ...
    bool miner::set_block_template(const Block &bl, const difficulty_type &di) {
        ...
        if (m_template.majorVersion >= BLOCK_MAJOR_VERSION_2) {
            Q1v::TransactionExtraMergeMiningTag mm_tag;
            mm_tag.depth = 0;
            if (!Q1v::get_aux_block_header_hash(m_template, mm_tag.merkleRoot)) {
                return false;
            }

            m_template.parentBlock.baseTransaction.extra.clear();
            if (!Q1v::appendMergeMiningTagToExtra(m_template.parentBlock.baseTransaction.extra, mm_tag)) {
                return false;
            }
        }
    ...
    void miner::merge_hr() {
        if (m_last_hr_merge_time && is_mining()) {
            ...
            if (m_do_print_hashrate) {
                uint64_t total_hr = std::accumulate(m_last_hash_rates.begin(), m_last_hash_rates.end(),
    ...
    bool miner::init(const MinerConfig &config) {
        if (!config.extraMessages.empty()) {
            ...
            std::string filebuf;
            if (Common::loadFileToString(m_config_folder_path + "/" + Q1v::parameters::MINER_CONFIG_FILE_NAME,
                                         filebuf)) {
                loadFromJson(m_config, filebuf);
            }

            logger(INFO) << "Loaded " << m_extra_messages.size() << " extra messages, current index "
                         << m_config.current_extra_message_index;
        }

        if (!config.startMining.empty()) {
            if (!m_currency.parseAccountAddressString(config.startMining, m_mine_address)) {
                logger(ERROR) << "Target account address " << config.startMining
        ...
        while (!m_stop) {
            ...
            if (!m_stop && check_hash(h, local_diff)) {
                ...
                if (!m_handler.handle_block_found(b)) {
                    ...
                    Common::saveStringToFile(m_config_folder_path + "/" + Q1v::parameters::MINER_CONFIG_FILE_NAME,
                                             storeToJson(m_config));
                }
            }
