...
namespace Q1v {
    ...
    public:
        ...
        TransactionImpl(const Q1v::Transaction &tx);
        ...
        virtual size_t
        addInput(const AccountKeys &senderKeys, const TransactionTypes::InputKeyInfo &info, KeyPair &ephKeys) override;

        virtual size_t addOutput(uint64_t amount, const AccountPublicAddress &to) override;

        virtual size_t
        addOutput(uint64_t amount, const std::vector<AccountPublicAddress> &to, uint32_t requiredSignatures) override;
        ...
        virtual void signInputMultisignature(size_t input, const PublicKey &sourceTransactionKey, size_t outputIndex,
    ...
    std::unique_ptr<ITransaction> createTransaction(const Q1v::Transaction &tx) {
        return std::unique_ptr<ITransaction>(new TransactionImpl(tx));
    }

    TransactionImpl::TransactionImpl() {
        Q1v::KeyPair txKeys(Q1v::generateKeyPair());
    ...
    TransactionImpl::TransactionImpl(const Q1v::Transaction &tx) : transaction(tx) {
        extra.parse(transaction.extra);
    }
    ...
    size_t TransactionImpl::addInput(const AccountKeys &senderKeys, const TransactionTypes::InputKeyInfo &info,
                                     KeyPair &ephKeys) {
        checkIfSigning();
        KeyInput input;
    ...
    size_t TransactionImpl::addOutput(uint64_t amount, const std::vector<AccountPublicAddress> &to,
    ...
    void
    TransactionImpl::signInputKey(size_t index, const TransactionTypes::InputKeyInfo &info, const KeyPair &ephKeys) {
        ...
        generate_ring_signature(reinterpret_cast<const Hash &>(prefixHash),
    ...
    void
    TransactionImpl::signInputMultisignature(size_t index, const PublicKey &sourceTransactionKey, size_t outputIndex,
                                             const AccountKeys &accountKeys) {
        KeyDerivation derivation;
        PublicKey ephemeralPublicKey;
        SecretKey ephemeralSecretKey;

        generate_key_derivation(reinterpret_cast<const PublicKey &>(sourceTransactionKey),
                                reinterpret_cast<const SecretKey &>(accountKeys.viewSecretKey), derivation);
        ...
        derive_secret_key(derivation, outputIndex, reinterpret_cast<const SecretKey &>(accountKeys.spendSecretKey),
                          ephemeralSecretKey);

        Signature signature;
        auto txPrefixHash = getTransactionPrefixHash();

        generate_signature(reinterpret_cast<const Hash &>(txPrefixHash), ephemeralPublicKey, ephemeralSecretKey,
                           signature);
    ...
    bool TransactionImpl::findOutputsToAccount(const AccountPublicAddress &addr, const SecretKey &viewSecretKey,
                                               std::vector<uint32_t> &out, uint64_t &amount) const {
        return ::Q1v::findOutputsToAccount(transaction, addr, viewSecretKey, out, amount);
    }
    ...
    bool TransactionImpl::validateInputs() const {
        return check_inputs_types_supported(transaction) && check_inputs_overflow(transaction) &&
               checkInputsKeyimagesDiff(transaction) && checkMultisignatureInputsDiff(transaction);
    }
