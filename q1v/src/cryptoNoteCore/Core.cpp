/**
 * Copyright (c) 2012-2016, The CryptoNote developers, The Bytecoin developers, The Bytecoin developers, The Karbowanec developers
...
//-----------------------------------------------------------------------------------
    ...
    bool
    core::get_blocks(uint32_t start_offset, uint32_t count, std::list<Block> &blocks, std::list<Transaction> &txs) {
    ...
    void core::getTransactions(const std::vector<Crypto::Hash> &txs_ids, std::list<Transaction> &txs,
                               std::list<Crypto::Hash> &missed_txs, bool checkTxPool) {
        ...
        for (const IBlock *block: chain) {
            ...
            for (size_t txNumber = 0; txNumber < block->getTransactionCount(); ++txNumber) {
                ...
                if (!handleIncomingTransaction(tx, txHash, blobSize, tvc, true)) {
                    logger(ERROR, BRIGHT_RED) << "core::addChain() failed to handle transaction " << txHash
            ...
            if (bvc.m_marked_as_orphaned || bvc.m_verifivation_failed) {
                logger(ERROR, BRIGHT_RED) << "core::addChain() failed to handle incoming block "
                                          << get_block_hash(block->getBlock()) << ", " << blocksCounter << "/"
                                          << chain.size();
                break;
            }
    ...
    bool core::handle_incoming_tx(const BinaryArray &tx_blob, tx_verification_context &tvc,
                                  bool keeped_by_block) { //Deprecated. Should be removed with CryptoNoteProtocolHandler.
    ...
    bool core::check_tx_semantic(const Transaction &tx, bool keeped_by_block) {
        ...
        if (amount_in < amount_out) {
            logger(ERROR) << "tx with wrong amounts: ins " << amount_in << ", outs " << amount_out
...
//}
    ...
    bool
    core::add_new_tx(const Transaction &tx, const Crypto::Hash &tx_hash, size_t blob_size, tx_verification_context &tvc,
    ...
    bool core::get_block_template(Block &b, const AccountPublicAddress &adr, difficulty_type &diffic, uint32_t &height,
...

        {
            ...
            if (b.majorVersion == BLOCK_MAJOR_VERSION_1) {
                ...
                if (m_currency.upgradeHeight(BLOCK_MAJOR_VERSION_6) == UpgradeDetectorBase::UNDEF_HEIGHT) {
                    b.minorVersion =
                            b.majorVersion == BLOCK_MAJOR_VERSION_5 ? BLOCK_MINOR_VERSION_1 : BLOCK_MINOR_VERSION_0;
                } else if (m_currency.upgradeHeight(BLOCK_MAJOR_VERSION_5) == UpgradeDetectorBase::UNDEF_HEIGHT) {
                    b.minorVersion =
                            b.majorVersion == BLOCK_MAJOR_VERSION_4 ? BLOCK_MINOR_VERSION_1 : BLOCK_MINOR_VERSION_0;
                } else if (m_currency.upgradeHeight(BLOCK_MAJOR_VERSION_4) == UpgradeDetectorBase::UNDEF_HEIGHT) {
                    b.minorVersion =
                            b.majorVersion == BLOCK_MAJOR_VERSION_3 ? BLOCK_MINOR_VERSION_1 : BLOCK_MINOR_VERSION_0;
            ...
            uint64_t blockchain_timestamp_check_window =
                    b.majorVersion < BLOCK_MAJOR_VERSION_5 ? parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW
                                                           : parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW_V5;
        ...
        bool r = m_currency.constructMinerTx(b.majorVersion, height, median_size, already_generated_coins, txs_size,
                                             fee, adr, b.baseTransaction, ex_nonce, 11);
        if (!r) {
        ...
        for (size_t try_count = 0; try_count != 10; ++try_count) {
            r = m_currency.constructMinerTx(b.majorVersion, height, median_size, already_generated_coins,
                                            cumulative_size, fee, adr, b.baseTransaction, ex_nonce, 11);
            ...
            if (coinbase_blob_size < cumulative_size - txs_size) {
                ...
                if (cumulative_size != txs_size + getObjectBinarySize(b.baseTransaction)) {
                    ...
                    if (cumulative_size != txs_size + getObjectBinarySize(b.baseTransaction)) {
                        ...
                        logger(TRACE, BRIGHT_RED) << "Miner tx creation have no luck with delta_extra size = " << delta
                                                  << " and " << delta - 1;
                        cumulative_size += delta - 1;
                        continue;
                    }
                    logger(DEBUGGING, BRIGHT_GREEN) << "Setting extra for block: " << b.baseTransaction.extra.size()
                                                    << ", try_count=" << try_count;
                }
            }
            if (!(cumulative_size == txs_size + getObjectBinarySize(b.baseTransaction))) {
    ...
    core::findBlockchainSupplement(const std::vector<Crypto::Hash> &remoteBlockIds, size_t maxCount,
    ...
    bool core::get_random_outs_for_amounts(const COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::request &req,
    ...
    bool
    core::handle_incoming_block_blob(const BinaryArray &block_blob, block_verification_context &bvc, bool control_miner,
                                     bool relay_block) {
    ...
    bool
    core::handle_incoming_block(const Block &b, block_verification_context &bvc, bool control_miner, bool relay_block) {
        if (control_miner) {
            pause_mining();
        }
        ...
        if (relay_block && bvc.m_added_to_main_chain) {
            ...
            if (!missed_txs.empty() && getBlockIdByHeight(get_block_height(b)) != get_block_hash(b)) {
                ...
                if (!(txs.size() == b.transactionHashes.size() && missed_txs.empty())) {
                    logger(ERROR, BRIGHT_RED) << "can't find some transactions in found block:" << get_block_hash(b)
                                              << " txs.size()=" << txs.size() << ", b.transactionHashes.size()="
                                              << b.transactionHashes.size() << ", missed_txs.size()"
                                              << missed_txs.size();
                    return false;
                }
    ...
    bool core::parse_tx_from_blob(Transaction &tx, Crypto::Hash &tx_hash, Crypto::Hash &tx_prefix_hash,
...
//}
    ...
    bool core::on_idle() {
        if (!m_starter_message_showed) {
            logger(INFO, CYAN) << "The node is synchronizing with the network. It may take up to several hours.";
            logger(INFO, CYAN)
                    << "You can set the level of log details through \"set_log <level>\" command*, where <level> is between 0 (no details) and 4 (very verbose).";
            logger(INFO, CYAN)
    ...
    bool core::queryBlocks(const std::vector<Crypto::Hash> &knownBlockIds, uint64_t timestamp, uint32_t &resStartHeight,
                           uint32_t &resCurrentHeight, uint32_t &resFullOffset, std::vector<BlockFullInfo> &entries) {
        ...
        uint32_t blocksLeft = static_cast<uint32_t>(std::min(BLOCKS_IDS_SYNCHRONIZING_DEFAULT_COUNT - entries.size(),
    ...
    bool core::findStartAndFullOffsets(const std::vector<Crypto::Hash> &knownBlockIds, uint64_t timestamp,
                                       uint32_t &startOffset, uint32_t &startFullOffset) {
        LockedBlockchainStorage lbs(m_blockchain);
    ...
    std::vector<Crypto::Hash> core::findIdsForShortBlocks(uint32_t startOffset, uint32_t startFullOffset) {
        ...
        if (startOffset < startFullOffset) {
            result = lbs->getBlockIds(startOffset,
    ...
    bool
    core::queryBlocksLite(const std::vector<Crypto::Hash> &knownBlockIds, uint64_t timestamp, uint32_t &resStartHeight,
    ...
    bool core::getBlockReward(uint8_t blockMajorVersion, size_t medianSize, size_t currentBlockSize,
        ...
        return m_currency.getBlockReward(blockMajorVersion, medianSize, currentBlockSize, alreadyGeneratedCoins, fee,
                                         reward, emissionChange);
    }

    bool core::scanOutputkeysForIndices(const KeyInput &txInToKey,
                                        std::list<std::pair<Crypto::Hash, size_t>> &outputReferences) {
        struct outputs_visitor {
            std::list<std::pair<Crypto::Hash, size_t>> &m_resultsCollector;

            outputs_visitor(std::list<std::pair<Crypto::Hash, size_t>> &resultsCollector) : m_resultsCollector(
                    resultsCollector) {}
    ...
    bool core::getMultisigOutputReference(const MultisignatureInput &txInMultisig,
    ...
    bool core::getBlocksByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t blocksNumberLimit,
                                    std::vector<Block> &blocks, uint32_t &blocksNumberWithinTimestamps) {
        std::vector<Crypto::Hash> blockHashes;
        if (!m_blockchain.getBlockIdsByTimestamp(timestampBegin, timestampEnd, blocksNumberLimit, blockHashes,
    ...
    bool core::getPoolTransactionsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd,
                                              uint32_t transactionsNumberLimit, std::vector<Transaction> &transactions,
                                              uint64_t &transactionsNumberWithinTimestamps) {
        std::vector<Crypto::Hash> poolTransactionHashes;
        if (!m_mempool.getTransactionIdsByTimestamp(timestampBegin, timestampEnd, transactionsNumberLimit,
                                                    poolTransactionHashes, transactionsNumberWithinTimestamps)) {
            return false;
        }
        std::list<Transaction> txs;
        std::list<Crypto::Hash> missed_txs;
    ...
    bool core::getTransactionsByPaymentId(const Crypto::Hash &paymentId, std::vector<Transaction> &transactions) {
        ...
        if (!poolTransactionHashes.empty()) {
            blockchainTransactionHashes.insert(blockchainTransactionHashes.end(), poolTransactionHashes.begin(),
    ...
    bool core::handleIncomingTransaction(const Transaction &tx, const Crypto::Hash &txHash, size_t blobSize,
                                         tx_verification_context &tvc, bool keptByBlock) {
    ...
    std::unique_ptr<IBlock> core::getBlock(const Crypto::Hash &blockId) {
        ...
        assert(missedTxs.empty() ||
               !lbs->isBlockInMainChain(blockId)); //if can't find transaction for blockchain block -> error
