...
namespace Q1v {
    ...
    public:
        OnceInTimeInterval(unsigned interval, Q1v::ITimeProvider &timeProvider) : m_interval(interval),
    ...
    private:
        ...
        unsigned m_interval;
        Q1v::ITimeProvider &m_timeProvider;
    };
    ...
    public:
        tx_memory_pool(const Q1v::Currency &currency, Q1v::ITransactionValidator &validator,
                       Q1v::ITimeProvider &timeProvider, Logging::ILogger &log, bool blockchainIndexesEnabled);
        ...
        bool add_tx(const Transaction &tx, const Crypto::Hash &id, size_t blobSize, tx_verification_context &tvc,
        ...
        bool
        fill_block_template(Block &bl, size_t median_size, size_t maxCumulativeSize, uint64_t already_generated_coins,
                            size_t &total_size, uint64_t &fee);

        void get_transactions(std::list<Transaction> &txs) const;

        void get_difference(const std::vector<Crypto::Hash> &known_tx_ids, std::vector<Crypto::Hash> &new_tx_ids,
        ...
        bool
        getTransactionIdsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit,
                                     std::vector<Crypto::Hash> &hashes, uint64_t &transactionsNumberWithinTimestamps);
    ...
    private:
        ...
        Q1v::ITransactionValidator &m_validator;
        Q1v::ITimeProvider &m_timeProvider;
