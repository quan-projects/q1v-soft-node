...
#include "../crypto/q1v_slow_hash.hpp"
...
namespace Q1v {
    ...
    public:
        ...
        uint64_t emissionReductorV6() const { return m_emissionReductorV6; }
        ...
        size_t difficultyBlocksCount2() const { return Q1v::parameters::DIFFICULTY_WINDOW_V2; }

        size_t difficultyBlocksCount4() const { return Q1v::parameters::DIFFICULTY_WINDOW_V4; }
        ...
        bool
        constructMinerTx(uint8_t blockMajorVersion, uint32_t height, size_t medianSize, uint64_t alreadyGeneratedCoins,
        ...
        bool
        isFusionTransaction(const std::vector<uint64_t> &inputsAmounts, const std::vector<uint64_t> &outputsAmounts,
                            size_t size) const;
        ...
        difficulty_type nextDifficulty(uint8_t blockMajorVersion, std::vector<uint64_t> timestamps,
                                       std::vector<difficulty_type> Difficulties) const;

        difficulty_type
        nextDifficultyV1(std::vector<uint64_t> timestamps, std::vector<difficulty_type> Difficulties) const;

        difficulty_type
        nextDifficultyV2(std::vector<uint64_t> timestamps, std::vector<difficulty_type> Difficulties) const;

        difficulty_type
        nextDifficultyV4(std::vector<uint64_t> timestamps, std::vector<difficulty_type> Difficulties) const;

        difficulty_type
        nextDifficultyByLWMA1(std::vector<uint64_t> timestamps, std::vector<difficulty_type> Difficulties) const;

        bool checkProofOfWorkV1(q1v_pow_hash_v2 &hash_ctx, const Block &block, difficulty_type currentDiffic,
                                Crypto::Hash &proofOfWork) const;

        bool checkProofOfWorkV2(q1v_pow_hash_v2 &hash_ctx, const Block &block, difficulty_type currentDiffic,
                                Crypto::Hash &proofOfWork) const;

        bool checkProofOfWork(q1v_pow_hash_v2 &hash_ctx, const Block &block, difficulty_type currentDiffic,
                              Crypto::Hash &proofOfWork) const;
    ...
    public:
        ...
        CurrencyBuilder &timestampCheckWindowV5(size_t val) {
            m_currency.m_timestampCheckWindowV5 = val;
            return *this;
        }
        ...
        CurrencyBuilder &blockFutureTimeLimitV4(uint64_t val) {
            m_currency.m_blockFutureTimeLimitV4 = val;
            return *this;
        }
        ...
        CurrencyBuilder &emissionReductorV6(uint64_t val);
        ...
        CurrencyBuilder &testnetFilenamePrefix(const std::string &val) {
            m_currency.m_testnetFilenamePrefix = val;
            return *this;
        }
