...
public:
    ...
    public:
        ...
        const_iterator(SwappedMap *swappedMap,
                       typename std::unordered_map<Key, Descriptor>::const_iterator descriptorsIterator) : m_swappedMap(
                swappedMap), m_descriptorsIterator(descriptorsIterator) {
        }
...
template<class Key, class T>
bool SwappedMap<Key, T>::open(const std::string &itemFileName, const std::string &indexFileName, size_t poolSize) {
    if (poolSize == 0) {
        return false;
    }
...
void SwappedMap<Key, T>::close() {
    std::cout << "SwappedMap cache hits: " << m_cacheHits << ", misses: " << m_cacheMisses << " (" << std::fixed
              << std::setprecision(2) << static_cast<double>(m_cacheMisses) / (m_cacheHits + m_cacheMisses) * 100
              << "%)" << std::endl;
}
...
void SwappedMap<Key, T>::erase(const_iterator iterator) {
    ...
    m_indexesFile.seekp(
            sizeof(uint64_t) + (sizeof(bool) + sizeof(Key) + sizeof(uint32_t)) * descriptorsIterator->second.index);
    bool valid = false;
...
template<class Key, class T>
std::pair<typename SwappedMap<Key, T>::const_iterator, bool>
SwappedMap<Key, T>::insert(const std::pair<const Key, T> &value) {
...
std::pair<const Key, T> *SwappedMap<Key, T>::prepare(const Key &key) {
    ...
    std::pair<typename std::unordered_map<Key, T>::iterator, bool> itemInsert = m_items.insert(
            std::make_pair(key, T()));
