...
namespace Q1v {
    ...
    //---------------------------------------------------------------------------------
    tx_memory_pool::tx_memory_pool(const Q1v::Currency &currency, Q1v::ITransactionValidator &validator,
                                   Q1v::ITimeProvider &timeProvider, Logging::ILogger &log,
                                   bool blockchainIndexesEnabled) : m_currency(currency), m_validator(validator),
    ...
    //---------------------------------------------------------------------------------
    bool tx_memory_pool::add_tx(const Transaction &tx, /*const Crypto::Hash& tx_prefix_hash,*/ const Crypto::Hash &id,
                                size_t blobSize, tx_verification_context &tvc, bool keptByBlock) {
        ...
        if (!keptByBlock && !isFusionTransaction && fee < m_currency.minimumFee()) {
            logger(INFO) << "transaction fee is not enough: " << m_currency.formatAmount(fee) << ", minimum fee: "
                         << m_currency.formatAmount(m_currency.minimumFee());
            tvc.m_verifivation_failed = true;
    ...
    //---------------------------------------------------------------------------------
    void
    tx_memory_pool::get_difference(const std::vector<Crypto::Hash> &known_tx_ids, std::vector<Crypto::Hash> &new_tx_ids,
    ...
    std::string tx_memory_pool::print_pool(bool short_format) const {
        ...
        for (const auto &txd: m_fee_index) {
            ...
            ss << "blobSize: " << txd.blobSize << std::endl << "fee: " << m_currency.formatAmount(txd.fee) << std::endl
               << "keptByBlock: " << (txd.keptByBlock ? 'T' : 'F') << std::endl << "max_used_block_height: "
               << txd.maxUsedBlock.height << std::endl << "max_used_block_id: " << txd.maxUsedBlock.id << std::endl
               << "last_failed_height: " << txd.lastFailedBlock.height << std::endl << "last_failed_id: "
               << txd.lastFailedBlock.id << std::endl << "amount_out: " << get_outs_money_amount(txd.tx) << std::endl
               << "fee_atomic_units: " << txd.fee << std::endl << "received_timestamp: " << txd.receiveTime << std::endl
...
#define CURRENT_MEMPOOL_ARCHIVE_VER 1
    ...
    void serialize(Q1v::tx_memory_pool::TransactionDetails &td, ISerializer &s) {
        s(td.id, "id");
    ...
    bool tx_memory_pool::removeExpiredTransactions() {
        bool somethingRemoved = false;
        {
            ...
            for (auto it = m_recentlyDeletedTransactions.begin(); it != m_recentlyDeletedTransactions.end();) {
                ...
                if (elapsedTimeSinceDeletion >
                    m_currency.numberOfPeriodsToForgetTxDeletedFromPool() * m_currency.mempoolTxLiveTime()) {
            ...
            for (auto it = m_transactions.begin(); it != m_transactions.end();) {
                ...
                bool remove = txAge > (it->keptByBlock ? m_currency.mempoolTxFromAltBlockLiveTime()
    ...
    tx_memory_pool::tx_container_t::iterator
    tx_memory_pool::removeTransaction(tx_memory_pool::tx_container_t::iterator i) {
        ...
        for (const auto &in: tx.inputs) {
            if (in.type() == typeid(KeyInput)) {
                ...
                if (!(it != m_spent_key_images.end())) {
                    logger(ERROR, BRIGHT_RED) << "failed to find transaction input in key images. img=" << txin.keyImage
                                              << std::endl << "transaction id = " << tx_id;
                    return false;
                }
                ...
                if (!(it_in_set != key_image_set.end())) {
                    logger(ERROR, BRIGHT_RED) << "transaction id not found in key_image set, img=" << txin.keyImage
                                              << std::endl << "transaction id = " << tx_id;
                    return false;
                }
        ...
        for (const auto &in: tx.inputs) {
            if (in.type() == typeid(KeyInput)) {
                ...
                if (!(keptByBlock || kei_image_set.size() == 0)) {
                    logger(ERROR, BRIGHT_RED) << "internal error: keptByBlock=" << keptByBlock
    ...
    bool tx_memory_pool::getTransactionIdsByPaymentId(const Crypto::Hash &paymentId,
                                                      std::vector<Crypto::Hash> &transactionIds) {
        std::lock_guard<std::recursive_mutex> lock(m_transactions_lock);
        return m_paymentIdIndex.find(paymentId, transactionIds);
    }

    bool tx_memory_pool::getTransactionIdsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd,
                                                      uint32_t transactionsNumberLimit,
                                                      std::vector<Crypto::Hash> &hashes,
                                                      uint64_t &transactionsNumberWithinTimestamps) {
        std::lock_guard<std::recursive_mutex> lock(m_transactions_lock);
        return m_timestampIndex.find(timestampBegin, timestampEnd, transactionsNumberLimit, hashes,
