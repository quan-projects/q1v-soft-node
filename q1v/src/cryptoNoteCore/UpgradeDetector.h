...
namespace Q1v {
    ...
    static_assert(Q1v::UpgradeDetectorBase::UNDEF_HEIGHT == UINT32_C(0xFFFFFFFF),
    ...
    public:
        BasicUpgradeDetector(const Currency &currency, BC &blockchain, uint8_t targetVersion, Logging::ILogger &log)
                : m_currency(currency), m_blockchain(blockchain), m_targetVersion(targetVersion),
                  m_votingCompleteHeight(UNDEF_HEIGHT), logger(log, "upgrade") {
        }
            ...
            if (upgradeHeight == UNDEF_HEIGHT) {
                if (m_blockchain.empty()) {
                    ...
                    auto it = std::lower_bound(m_blockchain.begin(), m_blockchain.end(), m_targetVersion,
                                               [](const typename BC::value_type &b, uint8_t v) {
                ...
                if (m_blockchain.size() <= upgradeHeight + 1) {
                    if (m_blockchain.back().bl.majorVersion >= m_targetVersion) {
                        logger(Logging::ERROR, Logging::BRIGHT_RED) << "Internal error: block at height "
                                                                    << (m_blockchain.size() - 1)
                                                                    << " has invalid version "
                    ...
                    if (blockVersionAtUpgradeHeight != m_targetVersion - 1) {
                        logger(Logging::ERROR, Logging::BRIGHT_RED) << "Internal error: block at height "
                                                                    << upgradeHeight << " has invalid version "
                                                                    << blockVersionAtUpgradeHeight << ", expected "
                                                                    << static_cast<int>(m_targetVersion - 1);
                        return false;
                    }
                    ...
                    if (blockVersionAfterUpgradeHeight != m_targetVersion) {
                        logger(Logging::ERROR, Logging::BRIGHT_RED) << "Internal error: block at height "
                                                                    << (upgradeHeight + 1) << " has invalid version "
                                                                    << blockVersionAfterUpgradeHeight << ", expected "
        ...
        uint64_t upgradeHeight() const {
            if (m_currency.upgradeHeight(m_targetVersion) == UNDEF_HEIGHT) {
                return m_votingCompleteHeight == UNDEF_HEIGHT ? UNDEF_HEIGHT : m_currency.calculateUpgradeHeight(
        ...
        void blockPushed() {
            ...
            if (m_currency.upgradeHeight(m_targetVersion) != UNDEF_HEIGHT) {
                ...
                if (m_blockchain.size() <= upgradeHeight()) {
                    ...
                    if (m_blockchain.size() % (60 * 60 / m_currency.difficultyTarget()) == 0) {
                        ...
                        logger(Logging::INFO, Logging::BRIGHT_GREEN)
                                ...
                                << " at about " << upgradeTimeStr << " (in " << Common::timeIntervalToString(interval)
                ...
                if (isVotingComplete(lastBlockHeight)) {
                    ...
                    logger(Logging::INFO, Logging::BRIGHT_GREEN) << "###### UPGRADE voting complete at block index "
                                                                 << m_votingCompleteHeight
        ...
        void blockPopped() {
            if (m_votingCompleteHeight != UNDEF_HEIGHT) {
                ...
                if (m_blockchain.size() == m_votingCompleteHeight) {
                    logger(Logging::INFO, Logging::BRIGHT_YELLOW) << "###### UPGRADE after block index "
                                                                  << upgradeHeight() << " has been canceled!";
                    m_votingCompleteHeight = UNDEF_HEIGHT;
                } else {
                    assert(m_blockchain.size() > m_votingCompleteHeight);
                }
        ...
        size_t getNumberOfVotes(uint64_t height) {
            ...
            if (m_blockchain.size() % 10 == 0) {
                logger(Logging::INFO, Logging::BRIGHT_YELLOW) << "#### Upgrade voting progress: " << voteCounter
                                                              << " votes out of "
                                                              << (m_currency.upgradeVotingThreshold() *
                                                                  m_currency.upgradeVotingWindow() / 100) << " needed.";
            }
            return voteCounter;
        }
    ...
    private:
        uint64_t findVotingCompleteHeight(uint64_t probableUpgradeHeight) {
            ...
            uint64_t probableVotingCompleteHeight =
                    probableUpgradeHeight > m_currency.maxUpgradeDistance() ? probableUpgradeHeight -
                                                                              m_currency.maxUpgradeDistance() : 0;
