...
namespace {
    ...
    struct BinaryVariantTagGetter : boost::static_visitor<uint8_t> {
        uint8_t operator()(const Q1v::BaseInput) { return 0xff; }

        uint8_t operator()(const Q1v::KeyInput) { return 0x2; }

        uint8_t operator()(const Q1v::MultisignatureInput) { return 0x3; }

        uint8_t operator()(const Q1v::KeyOutput) { return 0x2; }

        uint8_t operator()(const Q1v::MultisignatureOutput) { return 0x3; }

        uint8_t operator()(const Q1v::Transaction) { return 0xcc; }

        uint8_t operator()(const Q1v::Block) { return 0xbb; }
    };

    struct VariantSerializer : boost::static_visitor<> {
        VariantSerializer(Q1v::ISerializer &serializer, const std::string &name) : s(serializer), name(name) {}
    ...
    void getVariantValue(Q1v::ISerializer &serializer, uint8_t tag, Q1v::TransactionInput &in) {
        switch (tag) {
            case 0xff: {
                Q1v::BaseInput v;
                serializer(v, "value");
    ...
    void getVariantValue(Q1v::ISerializer &serializer, uint8_t tag, Q1v::TransactionOutputTarget &out) {
        switch (tag) {
            case 0x2: {
                Q1v::KeyOutput v;
                serializer(v, "data");
    ...
    template<typename T>
    bool serializePod(T &v, Common::StringView name, Q1v::ISerializer &serializer) {
        return serializer.binary(&v, sizeof(v), name);
    }

    bool serializeVarintVector(std::vector<uint32_t> &vector, Q1v::ISerializer &serializer, Common::StringView name) {
