...
namespace Q1v {
    ...
    public:
        ...
        void
        performAsync(BlockchainExplorer::AsyncContextCounter &asyncContextCounter, const INode::Callback &callback) {
    ...
    bool BlockchainExplorer::PoolUpdateGuard::beginUpdate() {
        auto state_ = m_state.load();
        for (;;) {
            switch (state_) {
                case State::NONE:
                    if (m_state.compare_exchange_weak(state_, State::UPDATING)) {
                        return true;
                    }
                    break;
                case State::UPDATING:
                    if (m_state.compare_exchange_weak(state_, State::UPDATE_REQUIRED)) {
                        return false;
                    }
                    break;
    ...
    bool BlockchainExplorer::PoolUpdateGuard::endUpdate() {
        ...
        for (;;) {
            ...
            if (m_state.compare_exchange_weak(state_, State::NONE)) {
                return state_ == State::UPDATE_REQUIRED;
            }
        }
    }
    ...
    BlockchainExplorer::BlockchainExplorer(INode &node, Logging::ILogger &logger) : node(node), logger(logger,
                                                                                                       "BlockchainExplorer"),
    ...
    bool BlockchainExplorer::addObserver(IBlockchainObserver *observer) {
        if (state.load() != INITIALIZED) {
            throw std::system_error(make_error_code(Q1v::error::BlockchainExplorerErrorCodes::NOT_INITIALIZED));
        }
    ...
    void BlockchainExplorer::init() {
        if (state.load() != NOT_INITIALIZED) {
            ...
            throw std::system_error(make_error_code(Q1v::error::BlockchainExplorerErrorCodes::ALREADY_INITIALIZED));
        }
        if (node.addObserver(this)) {
            ...
            throw std::system_error(make_error_code(Q1v::error::BlockchainExplorerErrorCodes::INTERNAL_ERROR));
        }
    ...
    bool BlockchainExplorer::getBlocks(const std::vector<uint32_t> &blockHeights,
        ...
        NodeRequest request(std::bind(static_cast<
                                              void (INode::*)(const std::vector<uint32_t> &,
                                                              std::vector<std::vector<BlockDetails>> &,
                                                              const INode::Callback &)
                                              >(&INode::getBlocks), std::ref(node), std::cref(blockHeights),
                                      std::ref(blocks), std::placeholders::_1));
    ...
    bool BlockchainExplorer::getBlocks(const std::vector<Hash> &blockHashes, std::vector<BlockDetails> &blocks) {
        ...
        NodeRequest request(std::bind(static_cast<
                                              void (INode::*)(const std::vector<Hash> &, std::vector<BlockDetails> &,
                                                              const INode::Callback &)
                                              >(&INode::getBlocks), std::ref(node),
    ...
    bool BlockchainExplorer::getBlocks(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t blocksNumberLimit,
        ...
        NodeRequest request(std::bind(static_cast<
                                              void (INode::*)(uint64_t, uint64_t, uint32_t, std::vector<BlockDetails> &,
                                                              uint32_t &, const INode::Callback &)
                                              >(&INode::getBlocks), std::ref(node), timestampBegin, timestampEnd,
                                      blocksNumberLimit, std::ref(blocks), std::ref(blocksNumberWithinTimestamps),
                                      std::placeholders::_1));
        std::error_code ec = request.performBlocking();
        if (ec) {
    ...
    bool BlockchainExplorer::getTransactions(const std::vector<Hash> &transactionHashes,
        ...
        NodeRequest request(std::bind(static_cast<
                                              void (INode::*)(const std::vector<Hash> &,
                                                              std::vector<TransactionDetails> &,
                                                              const INode::Callback &)
                                              >(&INode::getTransactions), std::ref(node),
                                      std::cref(reinterpret_cast<const std::vector<Hash> &>(transactionHashes)),
                                      std::ref(transactions), std::placeholders::_1));
        std::error_code ec = request.performBlocking();
        if (ec) {
    ...
    bool BlockchainExplorer::getPoolTransactions(uint64_t timestampBegin, uint64_t timestampEnd,
        ...
        NodeRequest request(std::bind(&INode::getPoolTransactions, std::ref(node), timestampBegin, timestampEnd,
                                      transactionsNumberLimit, std::ref(transactions),
                                      std::ref(transactionsNumberWithinTimestamps), std::placeholders::_1));
        std::error_code ec = request.performBlocking();
        if (ec) {
    ...
    bool BlockchainExplorer::getTransactionsByPaymentId(const Hash &paymentId,
        ...
        NodeRequest request(std::bind(&INode::getTransactionsByPaymentId, std::ref(node),
                                      std::cref(reinterpret_cast<const Hash &>(paymentId)), std::ref(transactions),
                                      std::placeholders::_1));
        std::error_code ec = request.performBlocking();
        if (ec) {
    ...
    bool BlockchainExplorer::isSynchronized() {
        ...
        NodeRequest request(
                std::bind(&INode::isSynchronized, std::ref(node), std::ref(syncStatus), std::placeholders::_1));
        std::error_code ec = request.performBlocking();
        if (ec) {
    ...
    void BlockchainExplorer::poolChanged() {
        ...
        NodeRequest request([this, rawNewTransactionsPtr, removedTransactionsPtr, isBlockchainActualPtr](
            ...
            node.getPoolSymmetricDifference(std::move(hashes), reinterpret_cast<Hash &>(knownBlockchainTop.hash),
                                            *isBlockchainActualPtr, *rawNewTransactionsPtr, *removedTransactionsPtr,
                                            callback);
        });
        ...
        request.performAsync(asyncContextCounter,
                             [this, rawNewTransactionsPtr, removedTransactionsPtr, isBlockchainActualPtr](
                                 ...
                                 ScopeExitHandler poolUpdateEndGuard(
                                         std::bind(&BlockchainExplorer::poolUpdateEndHandler, this));
                                 ...
                                 for (const Hash hash: *removedTransactionsPtr) {
                                     ...
                                     if (iter != knownPoolState.end()) {
                                         removedTransactionsHashesPtr->push_back({hash,
                                 ...
                                 NodeRequest request(std::bind(static_cast<
                                                               ...
                                                               std::ref(*newTransactionsPtr), std::placeholders::_1));

                                 request.performAsync(asyncContextCounter,
                                                      [this, newTransactionsHashesPtr, newTransactionsPtr, removedTransactionsHashesPtr](
                                                              std::error_code ec) {
                                                          ScopeExitHandler poolUpdateEndGuard(
                                                                  std::bind(&BlockchainExplorer::poolUpdateEndHandler,
                                                                            this));
                                                          ...
                                                          if (!newTransactionsPtr->empty() ||
                                                              ...
                                                              observerManager.notify(&IBlockchainObserver::poolUpdated,
                                                                                     *newTransactionsPtr,
    ...
    void BlockchainExplorer::blockchainSynchronized(uint32_t topHeight) {
        ...
        NodeRequest request(std::bind(static_cast<
                                              void (INode::*)(const std::vector<uint32_t> &,
                                                              std::vector<std::vector<BlockDetails>> &,
                                                              const INode::Callback &)
                                              >(&INode::getBlocks), std::ref(node), std::cref(*blockHeightsPtr),
                                      std::ref(*blocksPtr), std::placeholders::_1));

        request.performAsync(asyncContextCounter, [this, blockHeightsPtr, blocksPtr, topHeight](std::error_code ec) {
            if (ec) {
            ...
            if (!gotMainchainBlock) {
                logger(ERROR)
                        << "Can't send blockchainSynchronized notification because can't get blockchain top: all blocks on height "
                        << topHeight << " are orphaned.";
                return;
            }
    ...
    void BlockchainExplorer::localBlockchainUpdated(uint32_t height) {
        ...
        request.performAsync(asyncContextCounter, [this, blockHeightsPtr, blocksPtr](std::error_code ec) {
            ...
            if (!gotTopMainchainBlock) {
                logger(ERROR)
                        << "Can't send localBlockchainUpdated notification because can't get blockchain top: all blocks on height "
                        << topHeight << " are orphaned.";
                return;
            }
