...
namespace Q1v {
    ...
    BlockchainExplorerDataBuilder::BlockchainExplorerDataBuilder(Q1v::ICore &core,
                                                                 Q1v::ICryptoNoteProtocolQuery &protocol) : core(core),
                                                                                                            protocol(
                                                                                                                    protocol) {
    }
    ...
    bool BlockchainExplorerDataBuilder::fillTxExtra(const std::vector<uint8_t> &rawExtra,
        ...
        for (const TransactionExtraField &field: txExtraFields) {
            if (typeid(TransactionExtraPadding) == field.type()) {
                ...
                extraDetails.nonce.push_back(Common::toHex(boost::get<TransactionExtraNonce>(field).nonce.data(),
                                                           boost::get<TransactionExtraNonce>(field).nonce.size()));
            }
        }
        return true;
    }
    ...
    bool BlockchainExplorerDataBuilder::fillBlockDetails(const Block &block, BlockDetails &blockDetails) {
        ...
        if (!core.getBackwardBlocksSizes(blockDetails.height, blocksSizes,
        ...
        if (!core.getBlockReward(block.majorVersion, blockDetails.sizeMedian, 0, prevBlockGeneratedCoins, 0, maxReward,
                                 emissionChange)) {
            return false;
        }

        if (!core.getBlockReward(block.majorVersion, blockDetails.sizeMedian, blockDetails.transactionsCumulativeSize,
        ...
        for (const Transaction &tx: found) {
            TransactionDetails transactionDetails_;
            if (!fillTransactionDetails(tx, transactionDetails_, block.timestamp)) {
                return false;
            }
            blockDetails.transactions.push_back(std::move(transactionDetails_));
            blockDetails.totalFeeAmount += transactionDetails_.fee;
        }
        return true;
    }
